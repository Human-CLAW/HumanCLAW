"""Step climb down per-action ControlNet.

Architecture matches the stair-up heuristic MLP branch, but this class gives the
downstairs skill a separate checkpoint namespace.
"""
...
class StepClimbDownCtrlDiT(nn.Module):
    """Condition a frozen MotionDiT on the requested climb-down geometry."""

    def __init__(self, base_dit: MotionDiT, hidden_dim: int = 512):
        """Freeze the base denoiser and construct the StepClimbDownCtrlDiT control layers."""
    ...
    def forward(
        ...
        target_hd_norm: torch.Tensor,
    ) -> torch.Tensor:
        """Predict climb-down motion conditioned on the requested step geometry."""
